    ...
    <section class="contact" id="contact">
        <div class="row">
            <form action="{{ route('carts.store', $product) }}" method="POST" id="cart-form">
                @csrf
                <div class="mb-4">
                    <img src="{{ asset('uploads/products/' . $product->foto) }}" height="200px" width="200px">
                </div>
                <h1>{{ $product->nama_barang }}</h1>
                <h3>Rp{{ $product->harga }}</h3>
                <hr>
                <p>Tersisa: {{ $product->stok }} pcs</p>
                <div class="inputBox">
                    <span class="fas fa-envelope"></span>
                    <input class="form-control @error('jumlah') is-invalid @enderror " value="{{ old('jumlah') }}"
                        id="jumlah" name="jumlah" type="number" placeholder="Jumlah yang akan dibeli">
                </div>
                @error('jumlah')
                    <div class="text-danger"><small>{{ $message }}</small></div>
                @enderror
                <input type="submit" value="Add to cart" class="btn btn-secondary rounded-pill py-2 px-4">
            </form>
        </div>
    </section>
    <!-- cart section ends -->
